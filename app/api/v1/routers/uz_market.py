...
class GenerateReportRequest(BaseModel):
    """Full 25-field wizard request matching frontend form."""
    # Block 1: OKED
    oked_section: str = Field("J", description="OKED section A-U")
    oked_division: str = Field("62", description="OKED division 01-99")
    oked_class: Optional[str] = Field(None, description="OKED class")
    ...
    # AI provider
    provider: str = Field("groq", description="groq or perplexity")
...
# ---- Generate Full Report (25 fields -> 12-section report) ----

@router.post("/generate-report", summary="Генерация полного AI-отчёта (25 полей → 12 разделов)")
async def generate_full_report(
    body: GenerateReportRequest,
    _u=Depends(get_current_user),
):
    """
    Принимает все 25 полей из 7-шагового Wizard на фронтенде.
    ...
    try:
        # Convert to dict for the service (matching MarketAnalysisRequest fields)
        request_data = body.dict(exclude={"provider"})
