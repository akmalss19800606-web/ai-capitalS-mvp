...
const STEPS=["1. Вид деятельности (ОКВЭД)","2. Инвестиционные параметры","3. Финансовые допущения","4. Региональный контекст","5. Рынок и конкуренция","6. Юридическая форма и налоги","7. Риск-профиль"];
const TIPS=["Выберите вид деятельности по классификатору ОКВЭД Узбекистана (21 секция A-U)","Объём инвестиций, валюта, горизонт, тип инвестиции и стадия проекта","Доля долга, ожидаемая ставка кредита, выручка и маржинальность за 1-й год","Регион, город, СЭЗ и индустриальная зона","Целевые рынки, ожидаемая доля, количество конкурентов","Форма собственности, налоговый режим, число сотрудников","Риск-профиль (1-10) и зависимость от импорта (%)"];
...
function apiCall(path:string,body?:any){
  ...
  return fetch(`${API}/api/v1${path}`,opts).then(async r=>{if(!r.ok) throw new Error(await r.text());return r.json();});
}
...
function Sel({children,...p}:any){return <select {...p} style={{width:"100%",padding:"10px 12px",border:`1px solid ${C.border}`,borderRadius:8,fontSize:14,background:"#fff",...(p.style||{})}}>{children}</select>;}
function Inp(p:any){return <input {...p} style={{width:"100%",padding:"10px 12px",border:`1px solid ${C.border}`,borderRadius:8,fontSize:14,boxSizing:"border-box",...(p.style||{})}}/>;}
...
function Chip({active,onClick,children}:{active:boolean;onClick:()=>void;children:any}){
  return <button type="button" onClick={onClick} style={{padding:"8px 14px",borderRadius:20,fontSize:13,fontWeight:600,cursor:"pointer",border:active?`2px solid ${C.primary}`:`1px solid ${C.border}`,background:active?C.infoBg:"#fff",color:active?C.primary:C.text}}>{children}</button>;
}
...
export default function MarketAnalysisPage(){
  ...
  async function exportFmt(fmt:string){
    ...
    try{const token=localStorage.getItem("token");
      ...
      const blob=await r.blob();const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`market-report.${fmt}`;a.click();URL.revokeObjectURL(url);
    }catch{alert(`Экспорт в ${fmt.toUpperCase()} пока недоступен на сервере`);}
  }
  ...
  function renderStep(){switch(step){
    case 0:return <div><div style={g2}>
      <div><Label text="Секция ОКВЭД (Level 1) *" tip="21 секция A-U"/><Sel value={f.oked_section} onChange={(e:any)=>s("oked_section",e.target.value)}>{OKED_SECTIONS.map(o=><option key={o.code} value={o.code}>{o.code} — {o.name}</option>)}</Sel></div>
    ...
    case 4:return <div>
      <div style={{marginBottom:16}}><Label text="Целевые рынки *" tip="Выберите один или несколько"/><div style={{display:"flex",gap:8}}>{[["domestic","🇺🇿 Внутренний"],["cis_export","🌐 Экспорт СНГ"],["global_export","🌍 Глобальный экспорт"]].map(([v,l])=><button key={v} type="button" onClick={()=>toggle("target_markets",v)} style={{padding:"10px 18px",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",flex:1,border:f.target_markets.includes(v)?`2px solid ${C.primary}`:`1px solid ${C.border}`,background:f.target_markets.includes(v)?C.infoBg:"#fff",color:f.target_markets.includes(v)?C.primary:C.text}}>{l}</button>)}</div></div>
      <div style={g2}>
  ...
  function renderReport(){
    ...
    return <div style={{marginTop:24}}>
      <div style={{display:"flex",gap:16,alignItems:"center",marginBottom:20,padding:16,background:C.card,borderRadius:12,border:`1px solid ${C.border}`}}>
        <div style={{padding:"12px 24px",borderRadius:12,background:rec==="invest"?C.successBg:rec==="hold"?C.warningBg:C.errorBg,border:`2px solid ${recColor(rec)}`,fontWeight:700,fontSize:18,color:recColor(rec)}}>{recLabel(rec)}</div>
        ...
        <div style={{flex:1}}/>
        <div style={{display:"flex",gap:8}}>{["pdf","docx","xlsx","md"].map(fmt=><button key={fmt} onClick={()=>exportFmt(fmt)} style={{padding:"8px 16px",borderRadius:8,border:`1px solid ${C.border}`,background:"#fff",cursor:"pointer",fontSize:13,fontWeight:600}}>📄 {fmt.toUpperCase()}</button>)}</div>
      </div>
  ...
  return <div style={{minHeight:"100vh",background:C.bg,padding:"2rem 1rem"}}><div style={{maxWidth:960,margin:"0 auto"}}>
    ...
    <div style={{background:C.card,borderRadius:12,border:`1px solid ${C.border}`,padding:"1.5rem",marginBottom:16}}>{renderStep()}</div>
    <div style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
      {step>0&&<button onClick={()=>setStep(step-1)} disabled={loading} style={{padding:"12px 24px",borderRadius:8,border:`1px solid ${C.border}`,background:"#fff",cursor:"pointer",fontSize:14,fontWeight:600}}>← Назад</button>}
