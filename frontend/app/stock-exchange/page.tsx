...
/* ─── Tab: Overview ─── */
function OverviewTab({ overview, st, kpiStyle, cardStyle  }: Record<string, unknown>) {
  return (
    <div>
      ...
      {overview?.most_traded?.length > 0 && (
        <div style={cardStyle}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {overview.most_traded.map((item: Record<string, unknown>, i: number) => (
...
/* ─── Tab: Issuers ─── */
function IssuersTab({ issuers, st, cardStyle  }: Record<string, unknown>) {
  const [filter, setFilter] = useState("");

  const filtered = issuers.filter((iss: Record<string, unknown>) =>
    ...
    <div>
      ...
      <div style={cardStyle}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {filtered.map((iss: Record<string, unknown>) => (
...
/* ─── Tab: Trades ─── */
function TradesTab({ trades, st, cardStyle  }: Record<string, unknown>) {
  return (
    <div style={cardStyle}>
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 800 }}>
          ...
          <tbody>
            {trades.map((tr: Record<string, unknown>, i: number) => (
              <tr key={i} style={{ borderBottom: "1px solid #F0F0F0" }}>
...
/* ─── Tab: Charts ─── */
function ChartsTab({ overview, issuers, st, cardStyle  }: Record<string, unknown>) {
  ...
  const barData = (overview?.most_traded || []).map((item: Record<string, unknown>) => ({
