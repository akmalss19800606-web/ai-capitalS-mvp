...
export default function ChartsPage() {
  ...
  // Data per tab
  const [waterfallData, setWaterfallData] = useState<unknown>(null);
  const [tornadoData, setTornadoData] = useState<unknown>(null);
  const [bubbleData, setBubbleData] = useState<unknown>(null);
  ...
  useEffect(() => {
    portfolios.list().then((r: unknown) => {
...
function WaterfallChart({ data }: { data: unknown }) {
  const items: unknown[] = data.items || [];
  const unit = data.unit || '';

  // Build bar data for waterfall using invisible + visible bars
  const chartData = items.map((item: unknown, idx: number) => {
      ...
      <ResponsiveContainer width="100%" height={420}>
        <BarChart data={chartData} margin={{ top: 10, right: 30, left: 20, bottom: 40 }}>
          ...
          <Bar dataKey="value" stackId="stack" radius={[4, 4, 0, 0]}>
            {chartData.map((entry: unknown, idx: number) => (
...
function TornadoChart({ data }: { data: unknown }) {
  const items: unknown[] = data.items || [];
  const base = data.base || 0;
  const unit = data.unit || '';

  // Transform for horizontal bar chart
  const chartData = items.map((item: unknown) => ({
    factor: item.factor,
...
function BubbleChart({ data }: { data: unknown }) {
  const items: unknown[] = data.items || [];
  const xLabel = data.xLabel || 'X';
  const yLabel = data.yLabel || 'Y';

  // Group by category
  const categories = [...new Set(items.map((i: unknown) => i.category))];
  ...
  return (
    <div style={card}>
        ...
        <ScatterChart margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
          ...
          {categories.map((cat: string) => (
            <Scatter
              key={cat}
              name={cat}
              data={items.filter((i: unknown) => i.category === cat)}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px', marginTop: '20px' }}>
        {categories.map((cat: string) => {
          const catItems = items.filter((i: unknown) => i.category === cat);
...
function HeatmapChart({ data }: { data: unknown }) {
  ...
  const values = cells.map((c: unknown) => c.value);
  ...
  function getCellData(row: string, col: string): unknown | null {
    return cells.find((c: unknown) => c.row === row && c.col === col) || null;
  }
  ...
  return (
    <div style={card}>
      ...
      <div style={{
        ...
        {rows.map((row) => {
          const rowCells = cells.filter((c: unknown) => c.row === row);
