  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-gray-400 mb-6">Моделирование на реальных данных баланса организации</p>

        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-gray-900 rounded-xl p-4">
            <label className="block text-sm font-medium text-gray-400 mb-2">Организация</label>
            <select value={selectedOrg||""} onChange={e=>setSelectedOrg(parseInt(e.target.value)||null)} className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-white">
              <option value="">Выберите...</option>
              {orgs.map(o=><option key={o.id} value={o.id}>{o.name}</option>)}
            </select>
            {balance&&(
              <div className="mt-3 text-sm">
                <div className="flex justify-between"><span className="text-gray-400">Активы:</span><span className="text-blue-400">{fmtNum(balance.total_assets)}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Обязательства:</span><span className="text-red-400">{fmtNum(balance.total_liabilities)}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Капитал:</span><span className="text-purple-400">{fmtNum(balance.total_equity)}</span></div>
              </div>
            )}
          </div>
          <div className="bg-gray-900 rounded-xl p-4">
            <label className="block text-sm font-medium text-gray-400 mb-2">Сценарий</label>
            <div className="space-y-1">
              {SCENARIOS.map(s=>(
                <button key={s.id} onClick={()=>setScenario(s.id)} className={`w-full text-left px-3 py-1.5 rounded text-sm ${scenario===s.id?"bg-blue-600":"bg-gray-800 hover:bg-gray-700"}`}>{s.icon} {s.name}</button>
              ))}
            </div>
          </div>
          <div className="bg-gray-900 rounded-xl p-4">
            <label className="block text-sm font-medium text-gray-400 mb-2">Тяжесть</label>
            <div className="space-y-2">
              {SEVERITY.map(s=>(
                <button key={s.value} onClick={()=>setSeverity(s.value)} className={`w-full text-left px-3 py-2 rounded text-sm ${severity===s.value?"bg-blue-600":"bg-gray-800 hover:bg-gray-700"}`}>
                  {s.label} (-{(s.pct*100)}%)
                </button>
              ))}
            </div>
          ...
          <div className="bg-gray-900 rounded-xl p-6">
            ...
            <div className="grid grid-cols-3 gap-4 mt-6">
              <div className="p-4 bg-gray-800 rounded-lg">
                <p className="text-gray-400 text-sm">Текущая ликвидность</p>
              ...
              <div className="p-4 bg-gray-800 rounded-lg">
                <p className="text-gray-400 text-sm">Коэффициент автономии</p>
              ...
              <div className="p-4 bg-red-900/30 rounded-lg">
                <p className="text-gray-400 text-sm">Потенциальные убытки</p>
