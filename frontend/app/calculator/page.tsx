...
export default function CalculatorPage() {
  ...
  const UZS_RATE = 12700;
  const BANK_RATE = 0.22;
  ...
  const calculate = async () => {
    ...
    const bankReturn = inv * Math.pow(1 + BANK_RATE, yrs);
    ...
    try {
      const res = await apiRequest('/ai/market-analysis', {
        method: 'POST',
        body: JSON.stringify({
          query: `Investment of $${inv} in ${industry} in Uzbekistan for ${yrs} years. Calculate ROI, breakeven, risks. Compare with bank deposit 22% and inflation 10%. Answer in Russian.`,
          language: 'ru'
        })
      });
      ...
      const chartData = Array.from({ length: yrs }, (_, i) => ({
        ...
        'Банковский депозит': Math.round(inv * Math.pow(1 + BANK_RATE, i + 1)),
      ...
      setResult({
        ...
        profitUZS: Math.round((investReturn - inv) * UZS_RATE),
        chartData,
  ...
  return (
    <div>
      ...
      <div style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '24px', border: '1px solid #e2e8f0', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', padding: '12px 16px', backgroundColor: '#f8fafc', borderRadius: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '12px', color: '#64748b' }}>🏦 Ставка банков УЗ: <b>22%</b></span>
          <span style={{ fontSize: '12px', color: '#64748b' }}>📈 Инфляция: <b>10%</b></span>
          <span style={{ fontSize: '12px', color: '#64748b' }}>💱 1 USD = <b>12,700 сум</b></span>
        </div>
        <button onClick={calculate} disabled={loading} style={{ padding: '11px 32px', borderRadius: '8px', backgroundColor: '#3b82f6', color: '#fff', border: 'none', fontSize: '14px', fontWeight: '600', cursor: 'pointer', opacity: loading ? 0.7 : 1 }}>
        ...
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
            {[
              ...
              { label: 'Банковский депозит', value: `$${result.bankReturn.toLocaleString()}`, sub: 'Ставка 22% годовых', color: '#3b82f6', bg: '#eff6ff' },
