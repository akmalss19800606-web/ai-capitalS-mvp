...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { labelKey: 'aiAssistant', path: '/ai-assistant',
  ...
  for (const group of NAV_GROUPS) {
    for (const item of group.items) {
      if (item.path === pathname || (item.path !== '/' && pathname.startsWith(item.path))) {
        const label = (t.nav.items as unknown)[item.labelKey] || item.labelKey;
  ...
  const renderNavGroups = (isMobile: boolean) => (
    <nav style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: `${spacing[2]} 0` }}>
      {NAV_GROUPS.map((group, gi) => {
        const groupTitle = (t.nav.groups as unknown)[group.titleKey] || group.titleKey;
        return (
